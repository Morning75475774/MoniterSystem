...
namespace HslCommunication.ModBus
{
    ...
    public class ModbusRtu : SerialDeviceBase<ReverseWordTransform>
    {
        ...
        {
            // 生成最终rtu指令
            byte[] buffer = ModbusInfo.PackCommandToRtu(address.CreateReadInputRegister(station, length));
            return OperateResult.CreateSuccessResult(buffer);
        }
        ...
        /// <param name="length">读取长度</param>
        /// <returns>带有成功标志的bool数组对象</returns>
        public OperateResult<bool[]> ReadRegesiter(string address, ushort length)
        {
        ...
        /// <param name="length">读取长度</param>
        /// <returns>带有成功标志的bool数组对象</returns>
        public OperateResult<bool[]> ReadInputRegister(string address, ushort length)
        {
            var read = ReadModBusBase(ModbusInfo.ReadInputRegister, address, length);
            if (!read.IsSuccess) return OperateResult.CreateFailedResult<bool[]>(read);
